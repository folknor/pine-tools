			...
			function_declarations: [
				{
					...
					description:
						"Parses and validates a Pine Script file using the project's validator, returning a list of errors.",
					parameters: {
						type: "OBJECT",
						properties: {
							file_path: {
								type: "STRING",
								description:
									"The absolute path to the .pine script file to be validated.",
							},
						},
						required: ["file_path"],
					},
...
rl.on("line", async (line) => {
	try {
		...
		if (
			request.tool_code &&
			request.tool_code.name === "validate_pine_script"
		) {
		...
		const errorResponse = {
			tool_result: {
				...
				output: JSON.stringify([
					{ line: 0, column: 0, message: `Server error: ${e.message}` },
				]),
				is_error: true,
			},
		};
