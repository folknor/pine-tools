...
criticalConstants.forEach(({ namespace, member }) => {
	...
	console.log(
		`  ${status} ${namespace}.${member}: ${isValid ? "VALID" : "INVALID"} (expected: ${expected ? "VALID" : "INVALID"})`,
	);
});
...
const expectedErrors = [
	{
		line: 3,
		pattern: /sometin|undefined function/i,
		description: "Undefined function sometin()",
	},
	{
		line: 4,
		pattern: /plot\.styl|unknown.*styl/i,
		description: "Invalid constant plot.styl",
	},
	{
		line: 5,
		pattern: /someUndefinedVar|undefined variable/i,
		description: "Undefined variable someUndefinedVar",
	},
	{
		line: 6,
		pattern: /math\.nonexistent|undefined function/i,
		description: "Undefined function math.nonexistent",
	},
	{
		line: 7,
		pattern: /style_invalid|unknown.*constant/i,
		description: "Invalid constant style_invalid",
	},
	{
		line: 8,
		pattern: /str\.invalid|undefined function/i,
		description: "Undefined function str.invalid",
	},
	{
		line: 9,
		pattern: /alertcondition|too many|parameters/i,
		description: "Too many parameters for alertcondition",
	},
];
...
const unexpectedErrors = invalidErrors.filter(
	(err) => !expectedErrors.some((expected) => expected.line === err.line),
);
...
console.log("\n📋 Quality Gates:");
console.log(
	`  ${qualityGates.falsePositivesZero ? "✅" : "❌"} False Positives = 0 (REQUIRED)`,
);
console.log(
	`  ${qualityGates.falseNegativesLow ? "✅" : "❌"} False Negatives < 5 (ACCEPTABLE)`,
);
console.log(
	`  ${qualityGates.functionsComplete ? "✅" : "❌"} Functions >= 457 (TARGET)`,
);
