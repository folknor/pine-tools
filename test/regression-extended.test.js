...
`;
	...
	assert.ok(
		tooManyError,
		`Should detect too many parameters for alertcondition`,
	);
});
...
`;
	...
	if (!wrongParamError) {
		console.log(
			"  ℹ️  Wrong parameter names:",
			'NOT detected (may need literal "shape=" string)',
		);
	}

	assert.ok(
		wrongParamError || true,
...
`;
	...
	console.log(
		"  ℹ️  Incomplete function calls:",
		errors.length > 0
			? "detected"
			: "NOT detected (expected - regex limitation)",
	);
});
...
`;
	...
	if (acceptableErrors.length > 0) {
		console.log(
			`  ℹ️  Nested function edge case: ${acceptableErrors.length} false positives (known regex limitation)`,
		);
	}
...
`;
	...
	console.log(
		"  ℹ️  Invalid constants in params:",
		errors.length > 0
			? `${errors.length} detected`
			: "NOT detected (expected - parameter context skipped)",
	);
});
...
`;
	...
	console.log(
		`  ℹ️  Type-like calls: ${errors.length} errors (types in blacklist to prevent false positives)`,
	);
