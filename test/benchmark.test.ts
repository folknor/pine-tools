...
describe("Validation Benchmark - Real Code", () => {
	const validFixture = path.join(
		__dirname,
		"../pinescripts/test-scripts/valid.pine",
	);
	const invalidFixture = path.join(
		__dirname,
		"../pinescripts/test-scripts/invalid.pine",
	);
	...
	describe("Valid Code Should Not Produce Errors", () => {
		...
		it("valid indicator declarations", () => {
			const indicatorLines = lines.filter((l) =>
				l.trim().startsWith("indicator("),
			);
			assert.ok(
				indicatorLines.length >= 3,
				"Should have multiple indicator examples",
			);
			...
			indicatorLines.forEach((line, idx) => {
				assert.ok(
					...
					`Line ${idx} should have indicator call`,
				);
			});
		});
		...
		it("valid alertcondition calls", () => {
			const alertLines = lines.filter((l) =>
				l.trim().startsWith("alertcondition("),
			);
			assert.ok(
				alertLines.length >= 2,
				"Should have multiple alertcondition examples",
			);
	...
	describe("Invalid Code Should Produce Errors", () => {
		...
		it("should have examples of missing required parameters", () => {
			const comments = lines.filter(
				(l) => l.includes("ERROR:") && l.includes("missing required"),
			);
			assert.ok(
				comments.length >= 4,
				"Should have multiple missing param examples",
			);
		});

		it("should have examples of too many parameters", () => {
			const comments = lines.filter(
				(l) => l.includes("ERROR:") && l.includes("Too Many"),
			);
		...
		it("should have examples of wrong parameter names", () => {
			const comments = lines.filter(
				(l) => l.includes("ERROR:") && l.includes("Wrong Parameter"),
			);
		...
		it("should detect undefined function in valid namespace", () => {
			...
			assert.ok(
				errorLine,
				"Should have test case for undefined namespaced function",
			);
			// Validator should flag: Undefined function 'ta.nonexistentfunction'
		});

		it("should detect invalid comma-separated var declarations", () => {
			const errorLine1 = lines.find((l) =>
				l.includes("var float invalidA = na, invalidB = na"),
			);
			const errorLine2 = lines.find((l) =>
				l.includes("var int badX = 0, badY = 0"),
			);
			assert.ok(
				errorLine1,
				"Should have test case for comma-separated var float",
			);
			assert.ok(
				errorLine2,
				"Should have test case for comma-separated var int",
			);
	...
	describe("Version Upgrade Readiness", () => {
		it("fixtures should be version-tagged for future comparison", () => {
			...
			assert.ok(
				validCode.includes("//@version=6"),
				"Valid fixture should specify v6",
			);
			assert.ok(
				invalidCode.includes("//@version=6"),
				"Invalid fixture should specify v6",
			);
		...
		it("should document expected error count for regression testing", () => {
			...
			const invalidCode = fs.readFileSync(invalidFixture, "utf-8");
			const errorComments = invalidCode
				.split("\n")
				.filter((l) => l.includes("ERROR:"));

			// v6 baseline: ~12 documented errors
			assert.ok(
				errorComments.length >= 10,
				"Should have at least 10 documented errors",
			);
	...
	describe("Coverage Metrics", () => {
		it("should test all 32 manually verified functions", () => {
			...
			assert.ok(
				coverage.length >= 12,
				`Should cover at least 12 critical functions, got ${coverage.length}`,
			);
		});
