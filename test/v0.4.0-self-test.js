...
const missingConstants = [...officialConstants].filter(
	(ns) => !implementedConstants.has(ns),
);
const extraConstants = [...implementedConstants].filter(
	(ns) => !officialConstants.has(ns),
);
...
if (extraConstants.length > 0) {
	console.log(
		`ℹ️  EXTRA: ${extraConstants.join(", ")} (barstate is both constant and variable)`,
	);
}
...
const missingBuiltins = [...officialBuiltins].filter(
	(v) => !implementedBuiltins.has(v),
);
const extraBuiltins = [...implementedBuiltins].filter(
	(v) => !officialBuiltins.has(v),
);
...
const missingKeywords = [...officialKeywords].filter(
	(k) => !implementedKeywords.has(k),
);
const extraKeywords = [...implementedKeywords].filter(
	(k) => !officialKeywords.has(k),
);
...
if (extraKeywords.length > 0) {
	console.log(
		`ℹ️  EXTRA: ${extraKeywords.join(", ")} (enum is both keyword and potential identifier)`,
	);
}
...
const missingOperators = [...officialOperators].filter(
	(op) => !implementedOperators.has(op),
);
...
if (falsePositives === 0) {
	console.log(
		`✅ Zero false positives on ${previouslyMissing.length} test cases`,
	);
} else {
...
const missingVarNs = [...officialVarNamespaces].filter(
	(ns) => !implementedVarNamespaces.has(ns),
);
...
const missingFuncNs = [...officialFuncNamespaces].filter(
	(ns) => !implementedFuncNamespaces.has(ns),
);
