...
### Requesting collections and other objects with `request.*()` calls {#requesting-collections-and-other-objects-with-request-calls}
...
#### Return the last state only {#return-the-last-state-only}
...
#### Return IDs on some bars {#return-ids-on-some-bars}
...
#### Minimize the total drawings stored for a chart {#minimize-the-total-drawings-stored-for-a-chart}
