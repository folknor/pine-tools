...
export function activate(context: vscode.ExtensionContext) {
		...
		if (shouldApply) {
			...
			if (assoc["*.pine"] !== "pine") {
				...
				try {
					...
					Promise.resolve(t as unknown as Promise<void>).then(
						() => undefined,
						() => undefined,
					);
				} catch {
		...
		vscode.languages.registerCompletionItemProvider(
			"pine",
			{
				provideCompletionItems(document, position) {
					...
					const timeoutMs = cfg.get<number>(
								...
								(res) => {
									...
									res.on("end", () => {
										try {
											...
											const extra = (parsed.suggestions || [])
												.slice(0, 20)
												.map(
													(s) =>
	...
	const runDiagnostics = (doc: vscode.TextDocument) => {
		...
		if (
			/ta\.change\s*\(/m.test(text) &&
			/\?\s*\(|\)\s*and|\)\s*or/.test(text)
		) {
			diags.push(
				createDocDiag(
					doc,
		...
		while (match !== null) {
			...
			if (!/\btimeframe\s*=/.test(fullCall)) {
				const pos = doc.positionAt(
					match.index + fullCall.indexOf("timeframe_gaps"),
				);
	...
	context.subscriptions.push(
		vscode.commands.registerCommand("pine.showDocs", async () => {
			...
			const wordRange = ed.document.getWordRangeAtPosition(
				ed.selection.active,
				/[A-Za-z_.]+/,
			);
			...
			const docText =
				typeof md === "string" ? md : (md as vscode.MarkdownString).value;
