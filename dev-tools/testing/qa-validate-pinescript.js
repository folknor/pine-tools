...
function validateFile(filePath) {
	...
	try {
		...
		const warnings = errors.filter(
			(e) => e.severity === 1 || e.message.includes("declared but never used"),
		);
		...
		if (realErrors.length === 0) {
			...
			console.log(
				`  ${colorize("✗ FAIL", colors.red)} - ${realErrors.length} critical error(s)`,
			);
			...
			if (Object.keys(errorGroups).length > 5) {
				console.log(
					`    ... and ${Object.keys(errorGroups).length - 5} more error types`,
				);
			}
		...
		console.log(
			`  ${colorize("✗ ERROR", colors.red)} - Failed to validate: ${error.message}`,
		);
		return {
			file: filePath,
			passed: false,
...
function main() {
	...
	if (process.argv.length > 2) {
		...
		if (fs.existsSync(specifiedFile)) {
			...
			console.log(
				`${colorize("Error:", colors.red)} File not found: ${specifiedFile}`,
			);
			process.exit(1);
		}
	} else {
	...
	if (failed === 0) {
		console.log(
			colorize(
				"\n✓ All PineScript files validated successfully!",
				colors.green,
			),
		);
		process.exit(0);
		...
		console.log(
			colorize("  Fix errors before committing or deploying", colors.yellow),
		);
		process.exit(1);
	}
}
