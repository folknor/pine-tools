...
console.log(
	`Total VariableDeclaration statements: ${varDeclarations.length}\n`,
);
...
problematicVars.forEach((varName) => {
	...
	if (undefinedErrors.length > 0) {
		console.log(
			`❌ '${varName}': ${undefinedErrors.length} undefined error(s)`,
		);
...
// Summary
console.log("\n=== Summary ===\n");
const foundInAST = problematicVars.filter((v) =>
	varDeclarations.find((vd) => vd.name === v),
).length;
...
console.log(
	`Variables with undefined errors: ${hasErrors}/${problematicVars.length}`,
);
