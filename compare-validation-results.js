...
const PINE_DIR = path.join(__dirname, 'pine');
...
function runCli(pineFilePath) {
	const tmpFile = path.join(os.tmpdir(), `pine-cli-${Date.now()}-${Math.random().toString(36).slice(2)}.json`);
	
	try {
		// Run CLI and redirect stdout to temp file, ignore stderr
		const result = spawnSync('node', [CLI_PATH, pineFilePath], {
			encoding: 'utf8',
		...
		// Parse JSON output to extract actual errors from result
		try {
			const jsonOutput = JSON.parse(output);
			const errors = jsonOutput.result?.errors || [];
			const warnings = []; // CLI doesn't output warnings yet
			return {
...
function compareResults(cliResult, authorityJson) {
	...
	const differences = {
		summary: {
			cli: { success: cliResult.success, errorCount: cliErrorCount, warningCount: cliResult.warnings.length },
			authority: { success: authorityJson.success, errorCount: authErrorCount, warningCount: authorityJson.warnings?.length || 0 }
		},
		discrepancies: []
	};
	...
	const firstErrorMatches = authErrorCount === 1 &&
		cliErrorCount >= 1 &&
	...
	if (firstErrorMatches || bothNoErrors) {
		...
		} else if (cliErrorCount > 0 && authErrorCount > 0 && !errorsMatch(cliResult.errors[0], authorityJson.errors[0])) {
			differences.discrepancies.push({
...
function processFile(jsonFilePath) {
	...
	if (!fs.existsSync(pineFilePath)) {
		console.log(`⚠️  Skipping ${jsonFilename}: pine file not found at ${pineFilePath}`);
		return null;
	}
	...
	if (differences.discrepancies.length === 0) {
		...
		console.log(`❌ ${jsonFilename}: ${differences.discrepancies.length} discrepancies`);
...
function main() {
	...
	console.log(`Matches: ${matchCount} (${((matchCount/processedCount)*100).toFixed(1)}%)`);
	console.log(`Mismatches: ${mismatchCount} (${((mismatchCount/processedCount)*100).toFixed(1)}%)`);
	console.log('='.repeat(60));
