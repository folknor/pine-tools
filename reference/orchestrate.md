...
no `pnpm install` download, and critically **no `pine-lint --tv`** (the TV
endpoint is an outbound call). The load-bearing consequence is twofold. First,
codex cannot add an npm package that is not already in `pnpm-lock.yaml` and the
local pnpm store - `pnpm install` cannot fetch it offline, so the build/test
fails. Second, codex cannot itself verify a TradingView-parity gate; it can run
the offline gates (`pnpm test`, `pnpm run lint:regression`) but any `--tv` probe
the spec relies on must already be recorded in the spec (or its cited INV) as a
dated verdict, because codex cannot re-run it. The same applies to any other
network operation (remote fetch, git push, external API call). If a spec brick
requires a brand-new external dependency or a fresh TV verdict, the orchestrator
must produce it - via a Claude side-step (Opus or Sonnet in the main session) -
BEFORE the step-4 codex implement run. Alternatively, write the spec so it
builds on packages already in the lockfile and on TV verdicts already probed. A
codex run that hits a missing package fails at build time, and one that needs an
un-recorded TV verdict cannot resolve it, with no path to recovery inside that
run; the mitigation belongs in the spec review (steps 2-3) or a pre-step-4
...
e.g. "item #N in TODO.md" or "investigations/INV###", in whatever form the
TODO document named at invocation makes addressable. The agent reads the item
at the source. Nothing more.
...
> are removed entirely from TODO.md, never marked done - resolved work lives in
> git log, investigations/, and gotchas/), and reconcile any other document
> that still describes the pre-landing state (TODO.md indexes, investigations/
> notes, gotchas/, CLAUDE.md/AGENTS.md, the relevant README). Do not touch the
> spec document itself, and never cite it from any document - it is deleted at
> landing, so point durable references at git history (plus the INV/gotcha that
> records the decision) instead. Report each document you changed and why. Do
> not commit. Do not launch any sub-agents.
...
1. `pnpm run check` (biome - format, lint, assist; the fmt-plus-lint gate)
2. `pnpm test` (build:tsc + vitest - the build and unit/fixture gate)
3. `pnpm run install:cli` then `pnpm run lint:regression` (rebuild the CLI
   bundle the corpus check spawns, then confirm zero new error appearances
   against the snapshot - the false-positive safety net). On a clean result,
   `pnpm run lint:snapshot` to lock the new baseline.
4. Delete the spec document (the orchestrator manages spec-doc lifecycle;
   durable findings settle into git history, plus `investigations/` and
   `gotchas/`, per repo convention)
5. Commit

No further validation here - the per-brick TradingView-parity gates already ran
in the spec's steps (steps 2-3 author them, steps 4-5 satisfy them); step 7 is
the format-and-green-tree gate, not a re-litigation of parity. The corpus-wide
`pnpm run lint:failures` / `lint:categorize` TV sweep is a periodic
re-baselining run (it hits TV ~750 times), not a per-commit gate.
